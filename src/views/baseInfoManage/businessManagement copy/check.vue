...
type checkFormType = {
  auditTypeCode?: number,
  uuid?: string,
  roldId?: number | string,
  auditResult: 2 | 3,
  auditRemark?: "审核通过" | "审核不通过"
}
...
watch(() => route.params, (val: propsType) => {
  ...
  checkForm.uuid = form.uuid
  checkForm.roldId = form.roldId
}, {
  immediate: true
})
...
const submit = () => {
  checkFormRef.value.validateFields().then(async () => {
    loading.value = true
    if (checkForm.auditResult === 2) {
      checkForm.auditRemark = '审核通过'
    } else if (checkForm.auditResult === 3) {
      checkForm.auditRemark = '审核不通过'
    }
