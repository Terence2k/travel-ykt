<template>
    ...
    <template #bodyCell="{ column, record, index }">
      ...
      <template v-if="column.key === 'action'">
        <div class="action-btns">
          <a @click="checkDetails(record.oid)" v-permission="'查看详情'">查看详情</a>
          <a @click="widthdraw(record)" v-permission="'撤回'" v-show="![3, 4].includes(record.auditStatus)">撤回</a>
          <a @click="audit(record.oid)" v-permission="'审核'" v-show="record.auditStatus === 1">审核</a>
          <a @click="addOrUpdate({ row: record, handle: 'update' })" v-permission="'编辑'"
            v-show="[3, 4].includes(record.auditStatus)">编辑</a>
        </div>
      </template>
    </template>
  </CommonTable>
  ...
  <CommonModal title="撤回确认" v-model:visible="modalVisible" @cancel="modalCancel" @close="modalCancel"
    :conform-text="'确认撤回'" @conform="widthdrawConform">
    您即将撤销公告{{`“${widthdrawName}”`}}，撤回后用户该公告用户不可见是否确定撤回?
  </CommonModal>
</template>
...
const onSearch = async () => {
  const { content, total } = await api.messageManagePcNoticeList(state.tableData.param)
...
const checkDetails = (oid: any) => {
  goTo('messageDetails', { oid })
}
const audit = (oid: any) => {
  goTo('messageAudit', { oid })
}
...
const addOrUpdate = ({ row, handle }: addInterface) => {
  if (handle === 'add') {
    state.isAdd = true
    goTo('messageAdd', { operation: handle })
  } else if (handle === 'update') {
    state.isAdd = false
    goTo('messageAdd', { operation: handle, oid: row.oid })
  }
}
const widthdrawOid = ref()
const widthdrawName = ref('')
const widthdraw = (record: any) => {
  modalVisible.value = true
  widthdrawOid.value = record.oid
  widthdrawName.value = record.noticeName
}
const widthdrawConform = () => {
  api.revokeNotice(widthdrawOid.value).then(() => {
