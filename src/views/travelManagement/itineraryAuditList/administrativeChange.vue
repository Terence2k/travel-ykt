<template>
        ...
        <template v-if="column.key === 'index'">
          ...
          <template v-if="column.key === 'action'">
            <div class="action-btns">
              <a @click="auditStatus(record)" v-if="record.auditInfo?.length" v-permission="'变更审核_去审核'">去审核</a>
            </div>
          </template>
				</template>
		</CommonTable>
		<CommonPagination
			:current="travelStore.auditList.administrativeChange.params.pageNo"
			:page-size="travelStore.auditList.administrativeChange.params.pageSize"
			:total="state.total"
			@change="onHandleCurrentChange"
			@showSizeChange="pageSideChange"
		/>
	</div>
  <BaseModal title="行程变更审核" v-model="changeAuditVisible" :width="1000">
		<div class="table_box">
  ...
  <BaseModal title="驳回确认" v-model="rejectAuditVisible">
    驳回 {{state.detail.travelName}} 的行程变更申请，填写驳回理由：
...
<script lang="ts" setup>
  ...
  const userInfo = getUserInfo();
	const state = reactive({
		total: computed(() => travelStore.auditList.administrativeChange.total),
		params: {
				pageNo: 1,
				pageSize: 10,
				status: 1
		},
		tableData: computed(() => travelStore.auditList.administrativeChange.list),
    detail: {} as any,
		columns: [
			{
					title: ' 序号 ',
  ...
  const sendAudit = (status: any) => {
    ...
    if (status == 2) {
      Modal.confirm({
        ...
        centered: true,
        icon: false,
        content: `您即将批准 ${state.detail.travelName} 申请的行程变更申请，请确认该行程单的变更内容是否合规？是否同意？`,
        onOk() {
          const queryData = {
  ...
  const getDetail = (id: any, row: any) => {
	...
	const onHandleCurrentChange = (e: any) => {
		travelStore.auditList.administrativeChange.params.pageNo = e
		onSearch()
	}
