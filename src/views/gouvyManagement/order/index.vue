<template>
			...
			<template #bodyCell="{ column, index, record }">
				<template v-if="column.key === 'action'">
					<div class="action-btns">
						...
						<a href="javascript:;" @click="strong(record)" v-permission="'去强刷'" v-if="(record.forceBrushStatus==1 || record.forceBrushStatus==2)">去强刷</a>
					</div>
				</template>
		...
		<div class="table_box">
			<table class="info_table" cellpadding="16px" border="1">
				...
				<tr class="row">
					...
					<td class="value">{{state.tableData.details.itineraryNo}}</td>
				</tr>
				<tr class="row">
					<td class="key">组团社</td>
				...
				<tr class="row">
					...
					<td class="value">{{ state.tableData.details.itineraryStartDate}}-{{state.tableData.details.itineraryEndDate}} (共{{dayjs(state.tableData.details.itineraryEndDate).diff(state.tableData.details.itineraryStartDate, 'day') }}天)</td>
				</tr>
				<tr class="row">
	...
	<BaseModal title="强刷提示" v-model="Visible" :width="400">
		<p>当前行程单已于 {{state.tableData.details.endDate}} 过期。古维订单还未核销，但其他订单有核销记录。系统将于 2022.10.12 09:00:23 对该古维订单做自动强刷。您无需手动操作。</p>
		<template v-slot:footer>
	...
	<BaseModal title="选择需要强刷的游客" v-model="selectStrongTouristVisible" :width="1000">
		<div class="div-select">
			...
			<p>全部游客{{state.touristNum}}名，<span v-if="state.checkStatus=='1'">已完成线下查验，共{{state.notPresentNum}}人未到场，有{{state.reduceNum}}人可减免。</span> <span v-else>未完成查验</span></p>
		</div>
		<CommonTable :row-selection="{ onSelect,onSelectAll}" :columns="strongColumns" :dataSource="state.strongTouristList" rowKey="oid" :scrollY="false" style="padding:0">
				<template #bodyCell="{ column, index, record }">
				</template>
			</CommonTable>
		<div class="checked-div">
			<a-checkbox v-model:checked="checked"><span>已阅读并知晓：线下查验时未到场的、已通过减免申请的、已购买的无需核销，其余游客均可勾选强刷。请检查确认无误，强刷后不可撤回。</span></a-checkbox>
		</div>
		<template v-slot:footer>
...
const countDown = (end:any) => {
     ...
     //所剩分钟换算，不足10时，前面补0
      let min =parseInt((changeTime / 1000 / 60) % 60) > 9? parseInt((changeTime / 1000 / 60) % 60)+'分' : '0' + parseInt((changeTime / 1000 / 60) % 60)+'分';
      // 所剩秒数换算，不足10时，前面补0
